...
export default function NotePage() {
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-sm md:max-w-xl lg:max-w-2xl flex flex-col gap-6 md:gap-8">
        {/* Authentic Ruled Legal Pad / Journal Note */}
        <div
          className="relative rounded-3xl md:rounded-[32px] bg-[#181B32] p-6 md:p-8 shadow-scrapbook border border-[#302B4D] overflow-hidden"
          style={{
        ...
        >
          ...
          <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 h-5 md:h-6 w-24 md:w-32 bg-[#7147A8]/70 backdrop-blur-xs shadow-2xs rotate-1 border-y border-[#B98AE8]/40 z-20 pointer-events-none" />
          ...
          <div className="flex justify-between items-center pb-3 mb-2 border-b border-dashed border-[#272A43]">
            <div className="flex gap-2.5 md:gap-3.5">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-3 w-3 md:h-3.5 md:w-3.5 rounded-full bg-[#0D1020] border border-[#272A43] shadow-inner" />
              ))}
            </div>
            <span className="text-[10px] md:text-xs font-mono text-[#918DA1] font-semibold tracking-wider">
              {noteData.memo.tag}
            </span>
          </div>

          {/* Red/Pink Vertical Margin Line & Note Content */}
          <div className="border-l-2 border-[#E875A6]/50 pl-4 md:pl-6 ml-1 space-y-4 md:space-y-5 text-[#F7F4FC]">
            <p className="font-handwriting text-3xl md:text-4xl font-bold text-[#F7F4FC] leading-snug">
              {noteData.memo.greeting}
            </p>

            <p className="text-xs md:text-sm text-[#C9C5D6] leading-loose md:leading-relaxed">
              {noteData.memo.intro}
            </p>

            <div className="rounded-xl bg-[#12152A] p-2.5 md:p-3.5 text-center border border-[#272A43] shadow-2xs">
              <span className="font-display text-sm md:text-base font-bold text-[#F7F4FC]">
            ...
            <div className="pt-2 md:pt-3 border-t border-[#272A43] flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <span className="font-handwriting text-2xl md:text-3xl font-bold text-[#F7F4FC]">
                  {noteData.memo.closingBold}
                </span>
                <div className="flex items-center gap-2">
                  <CollectibleSticker id="tanisha_smile" size={54} rotation={-4} />
                  <Heart className="h-4 w-4 md:h-5 md:w-5 text-[#E875A6] fill-[#E875A6] animate-pulse" />
                </div>
              </div>
              <p className="whitespace-pre-line text-xs md:text-sm font-handwriting font-bold text-[#FFB6D5] leading-relaxed">
        ...
        {noteData.polaroidKeepsake && (
          <div className="relative self-center w-full max-w-[320px] md:max-w-[420px] lg:max-w-[460px] my-1 md:my-3">
            <PhotoCard
        ...
        {(noteData as any).hackathonMemory && (
          <div className="relative rounded-3xl bg-sky-800 p-5 md:p-7 shadow-scrapbook border border-purple-deep/30 text-left">
            <span className="absolute -top-3 left-6 h-5 md:h-6 w-20 md:w-28 bg-blue-deep/60 border border-blue-light/40 rounded-xs -rotate-2" />
            ...
            <p className="whitespace-pre-line font-handwriting text-lg md:text-xl text-[#D0CDDC] leading-relaxed my-2 md:my-3">
              {(noteData as any).hackathonMemory.story}
            </p>
            <div className="pt-2 border-t border-purple-deep/20 flex justify-end">
              <span className="text-[10px] md:text-xs font-mono font-semibold text-[#9693A7]">
        ...
        <div className="relative rounded-3xl bg-sky-800 p-5 md:p-7 shadow-scrapbook border border-purple-deep/30">
          <h4 className="font-display text-xs md:text-sm font-bold text-[#F7F5FC] uppercase tracking-wider text-center mb-3 md:mb-4">
            {noteData.whyItExists.title}
          </h4>

          <div className="flex flex-col items-center gap-2 md:gap-3 text-center text-xs md:text-sm text-[#F7F5FC] font-medium">
            {noteData.whyItExists.steps.map((step, idx) => (
              <React.Fragment key={idx}>
                {idx > 0 && <ArrowDown className="h-3 w-3 md:h-4 md:w-4 text-[#9693A7]" />}
                <span
                  className={
                    step.style === "cream"
                      ? "rounded-full bg-sky-850 px-3 md:px-4 py-1 md:py-1.5 shadow-2xs border border-purple-deep/30 text-[#F7F5FC]"
                      : step.style === "yellow"
                      ? "rounded-full bg-blue-night px-3 md:px-4 py-1 md:py-1.5 border border-blue-deep text-blue-light"
                      : step.style === "handwriting"
                      ? "font-handwriting text-base md:text-lg font-bold text-[#F7F5FC]"
                      : "rounded-full bg-purple-night px-4 md:px-5 py-1.5 md:py-2 font-bold text-[#F7F5FC] border border-purple-deep shadow-xs"
                  }
                >
                  {step.text}
                </span>
          ...
          <div className="mt-5 md:mt-6 pt-3 md:pt-4 border-t border-purple-deep/20 text-center flex flex-col items-center">
            <p className="text-[10px] md:text-xs text-purple-light font-mono font-bold uppercase tracking-wider mb-2">
              {noteData.whyItExists.questBadge}
            </p>
            <div className="my-1">
              <CollectibleSticker id="tanisha_bye" size={62} rotation={4} showTapPrompt={true} />
            </div>
            <p className="text-[10px] md:text-xs text-[#9693A7] italic mt-1">
        ...
        {/* Tucked Notice */}
        <div className="rounded-2xl bg-blue-night/40 p-3.5 md:p-5 border border-blue-deep/40 text-center">
          <p className="font-handwriting text-base md:text-lg text-[#F7F5FC] font-bold">
            {noteData.tuckedNotice.quote}
          </p>
          <span className="text-[11px] md:text-xs text-[#D0CDDC]">
